generate <- function(mu_g, mu_b, sigma_g, sigma_b, b, n) {
  ...
  final_g <- cbind(x_g, Y = y_g)
  final_b <- cbind(x_b, Y = y_b)
...
# Function based on the Galimard implementation
galimard_option_2 <- function (data, varMNAR, m, test_data){
  ...
  JointModelEq[,"Y_hat_var_sel"] <- c(1,1,1,1,1,0)
  JointModelEq[,"Y_hat_var_out"] <- c(1,1,1,1,0,0)
  ...
  fit <- with(imp, glm(Y_hat ~ V1 + V2, family=binomial(link = "probit")))
...
# Function based on the PPMA paper
...
complete_case_func <- function(data, varMNAR, test_data){
  ...
  formula <- as.formula(paste(varMNAR, "~ V1+V2"))
  fit_cca <- glm(formula, family=binomial, data = train_data_cca)
  pred = predict(fit_cca, newdata = test_data[,c("V1", "V2")], type = "response")
  require(PRROC)
  require(Metrics)
  # Compute ROC curve
  ...
  # Print AUC
  auc_pr = pr.curve(scores.class0 = test_data[, c("Y")] , weights.class0 = pred)$auc.integral
  print(paste("AUC_ROC from CCA:", auc_value))
  print(paste("AUC_PR from CCA:", auc_pr))
...
oracle_model_func <- function(data, varMNAR, test_data){
  ...
  pred = predict(fit_oracle, newdata = test_data[,c("V1", "V2")], type = "response")
  require(PRROC)
  require(Metrics)
  # Compute ROC curve
  #roc_curve <- roc(response = test_data[, c("Y")], predictor = pred)
  ...
  print(paste("AUC_PR from Oracle:", auc_pr))
