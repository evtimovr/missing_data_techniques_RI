...
simulation_gal(mu_g, mu_b, sigma_g, sigma_b, n_train, n_test, b, alpha, j, i)
}
...
simulation_gal <- function(mu_g, mu_b, sigma_g, sigma_b, n_train, n_test, b, alpha, j_max, i) {
  ...
  for (j in 1:j_max) {
    ...
    if (j %% 50 == 0) {
      ...
      auc_score_gal = galimard_option_2(D_gal,'Y',5, D)
      ...
      sim_df <- rbind(sim_df, data.frame(combination = i, iteration = j, auc_a = auc(D$Y,D$Y_pred), 
                                           auc_o = auc(D$Y,D$Y_pred_fo), auc_gal = auc_score_gal, auc_ppma = auc_score_ppma))
      print(sim_df)
    }
