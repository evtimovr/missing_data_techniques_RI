...
galimard_option_2 <- function (data, varMNAR, m, test_data){
  ...
  JointModelEq[,"Y_var_sel"] <- c(1,1,1,1,0)
  JointModelEq[,"Y_var_out"] <- c(1,1,1,0,0)
  ...
  fit <- with(imp, glm(Y ~ V1 + V2 + V4 + V5, family=binomial(link = "probit")))
  
  
  
  # trying to pool the results after getting them for each of the models 
  # fitted to each imputed dataset
  predm <- lapply(getfit(fit), predict, newdata= test_data [, c("V1", "V2", "V4", "V5")], se.fit = TRUE, type = "response")
...
PPMA_MI_sim <- function(data, varMNAR, m, test_data, phi) {
  ...
  fit <- with(modified_data_0, glm(Y ~ V1+V2+V4+V5, family=binomial))
